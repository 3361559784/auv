#!/usr/bin/env node
/**
 * PostToolUse Hook: Accumulate edited JS/TS/Rust file paths for batch processing
 *
 * Cross-platform (Windows, macOS, Linux)
 *
 * Records each edited path to a session-scoped temp file (one path per line).
 * stop-format-rust.js and stop-format-typecheck.js read this list at Stop time
 * and run format checks once across all edited files, eliminating per-edit
 * latency.
 *
...
function appendPath(filePath) {
  if (filePath && (JS_TS_EXT.test(filePath) || RUST_EXT.test(filePath))) {
