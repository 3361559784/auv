const MACOS_NATIVE_FFI_RS: &str = "src/driver/macos/native/ffi.rs";
const MACOS_NATIVE_SWIFT_PACKAGE: &str = "src/driver/macos/native/swift/Package.swift";
const MACOS_NATIVE_SWIFT_TARGET_DIR: &str = "src/driver/macos/native/swift/Sources/AuvMacosNative";
...
fn build_macos_native() {
  ...
  let crate_bridge_dir = generated_dir.join("auv_cli");
  ...
  swift_bridge_build::parse_bridges(vec![manifest_dir.join(MACOS_NATIVE_FFI_RS)])
    .write_all_concatenated(&generated_dir, "auv_cli");
  ...
  fs::write(
    ...
    format!(
      ...
      crate_bridge_dir.join("auv_cli.h").display()
    ),
  )
  ...
  let status = command
    .arg(crate_bridge_dir.join("auv_cli.swift"))
    .arg("-o")
    .arg(&swift_lib)
    .status()
    .expect("spawn swiftc");
