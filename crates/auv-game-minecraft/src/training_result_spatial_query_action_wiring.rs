...
pub const MC19_V1_D1_QUERY_ACTION_WIRING_KNOWN_LIMIT: &str =
  "mc19_v1_d1_query_action_wiring_seam_no_live_click_telemetry";
...
pub fn wire_query_manifest_to_action(
  ...
  known_limits.push(MC19_V1_D1_QUERY_ACTION_WIRING_KNOWN_LIMIT.to_string());
...
mod tests {
  ...
  fn click_ready_invokes_executor_once() {
    ...
    assert!(
      outcome
        .known_limits
        .iter()
        .any(|limit| limit == MC19_V1_D1_QUERY_ACTION_WIRING_KNOWN_LIMIT)
    );
  }
  ...
  fn click_ready_missing_window_point_defensively_refuses() {
    ...
    let outcome = wire_readiness_to_action(
      &readiness,
      &lineage,
      vec![MC19_V1_D1_QUERY_ACTION_WIRING_KNOWN_LIMIT.to_string()],
      &executor,
    );
